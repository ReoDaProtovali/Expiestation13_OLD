...
**Modularization** means, that most of the changes and additions we do, will be kept in a separate **`expiestation/`** folder, as independent from the core code as possible, and those which absolutely cannot be modularized, will need to be properly marked by comments. More details will be found in the next section.
...
The `/expiestation` folder will look fairly similar to tgstation's, following the same file structure. Thus, if you are modularizing something, attempt to mirror the tgcode file structure in the `/expiestation` folder as best as possible.
...
All assets added should be placed in the `/expiestation` folder.
...
- **_Example:_** You're adding a new lavaland mob.
  ...
  Next, you want to mirror that in the `/expiestation` folder, making it `/expiestation/code/modules/mob/living/simple_animal/hostile/mining_mobs/your_new_mob_here.dm`

  Then, you add the new mob to that new file in the `/expiestation` folder, like so.

  ```byond
    /mob/lavaland/newmob
      icon = 'expiestation/icons/mob.dmi'
      icon_state = "dead_1"
      sound = 'expiestation/sounds/boom.ogg'
  ```
...
- **Addition:**
  ...
  ```byond
  //EXPIESTATION EDIT START
  var/adminEmergencyNoRecall = FALSE
  var/lastMode = SHUTTLE_IDLE
  var/lastCallTime = 6000
  //EXPIESTATION EDIT END
  ```

- **Removal:**

  ```byond
  //EXPIESTATION REMOVAL START
  /*
  for(var/obj/docking_port/stationary/S in stationary)
    if(S.id = id)
      return S
  */
  //EXPIESTATION REMOVAL END
...
- **Change:**
  ...
  ```byond
  //EXPIESTATION EDIT START
  //if(SHUTTLE_STRANDED, SHUTTLE_ESCAPE) - EXPIESTATION EDIT ORIGINAL
